...
static int svc_cb(uint16_t conn_handle, const struct ble_gatt_error* error, const struct ble_gatt_svc* service, void* arg) {
    ...
    TEST_ASSERT_EQUAL_HEX(0, error->status);
    TEST_ASSERT(service);
    TEST_ASSERT_EQUAL_HEX(0, ble_gattc_read_by_uuid(conn_handle, service->start_handle, service->end_handle, &hello_chr.u, read_cb, NULL));
    return 0;
}
...
TEST_CASE("BLE hello endpoint (uuid)", "[ble][hello]") {
    TEST_IGNORE_MESSAGE("reading with UUID is currently broken in upstream NimBLE");
