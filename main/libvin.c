...
static bool read_vin_09(struct shared_data* shared) {
    ...
    omni_debug_log("VIN", "Attempting VIN read using Service 09");
    ...
    if (can_transmit(&request)) {
        ...
        omni_debug_log("VIN", "Service 09 request sent successfully");
...
static bool read_vin_22(struct shared_data* shared) {
    ...
    assert(shared);
    omni_debug_log("VIN", "Attempting VIN read using Service 22");
    ...
    debug_log_message("transmit", &request);
    if (can_transmit(&request)) {
        omni_debug_log("VIN", "Service 22 request sent successfully");
        // ESP_LOGI(tag, "sent request");
        static const twai_message_t ff_pattern = {
            .identifier = 0x7e8,
            .data_length_code = 8,
        ...
        if (generic_match(&ff_pattern, 5)) {
            omni_debug_log("VIN", "Service 22 first frame matched");
            // ESP_LOGI(tag, "successful response");
            memcpy(shared->vin, message.data + 5, 3);
            debug_log_message("transmit", &generic_continuation);
            if (can_transmit(&generic_continuation)) {
                // ESP_LOGI(tag, "sent continuation");
                omni_debug_log("VIN", "Service 22 continuation sent");
                return generic_read_continuation(shared, 14);
            }
            omni_debug_log("VIN", "Service 22 continuation send failed");
            // ESP_LOGE(tag, "failed to send continuation");
            return false;
        }
        omni_debug_log("VIN", "Service 22 response pattern mismatch");
        // ESP_LOGE(tag, "failed to receive response");
        return false;
    }
    // ESP_LOGE(tag, "failed to send request");
    omni_debug_log("VIN", "Service 22 request failed");
    return false;
}
...
static bool read_vin_1a(struct shared_data* shared) {
    ...
    omni_debug_log("VIN", "Attempting VIN read using Service 1A");
    twai_clear_receive_queue();
    twai_clear_transmit_queue();
    static const twai_message_t request = {
        .identifier = 0x7e0,
    ...
    debug_log_message("transmit", &request);
    if (can_transmit(&request)) {
        omni_debug_log("VIN", "Service 1A request sent successfully");
        // ESP_LOGI(tag, "sent request");
        static const twai_message_t ff_pattern = {
            .identifier = 0x7e8,
            .data_length_code = 8,
        ...
        if (generic_match(&ff_pattern, 4)) {
            omni_debug_log("VIN", "Service 1A first frame matched");
            // ESP_LOGI(tag, "successful response");
            memcpy(shared->vin, message.data + 4, 4);
            debug_log_message("transmit", &generic_continuation);
            if (can_transmit(&generic_continuation)) {
                omni_debug_log("VIN", "Service 1A continuation sent");
                // ESP_LOGI(tag, "sent continuation");
                return generic_read_continuation(shared, 13);
            }
            omni_debug_log("VIN", "Service 1A continuation send failed");
            // ESP_LOGE(tag, "failed to send continuation");
            return false;
        }
        omni_debug_log("VIN", "Service 1A response pattern mismatch");
        // ESP_LOGE(tag, "failed to receive response");
        return false;
    }
    omni_debug_log("VIN", "Service 1A request failed");
    // ESP_LOGE(tag, "failed to send request");
    return false;
}
...
static void try_read_vin(void* arg) {
    assert(arg);
    omni_debug_log("VIN", "VIN reader task started");
    ...
    assert(bt == pdTRUE);
    omni_debug_log("VIN", "VIN mutex acquired");
    // ESP_LOGI(tag, "mutex taken");

    for (;;) {
        if (read_vin_09(shared)) {
            omni_debug_log("VIN", "Successfully read VIN using Service 09");
            break;
        }
        if (read_vin_22(shared)) {
            omni_debug_log("VIN", "Successfully read VIN using Service 22");
            break;
        }
        if (read_vin_1a(shared)) {
            omni_debug_log("VIN", "Successfully read VIN using Service 1A");
            break;
        }
        omni_debug_log("VIN", "All VIN read methods failed, retrying in 3s");
        // ESP_LOGE(tag, "failed to read VIN, trying again in 3 seconds");
        vTaskDelay(pdMS_TO_TICKS(3000));
    }

    xSemaphoreGive(shared->semaphore);
    omni_debug_log("VIN", "VIN read complete, mutex released");
    ...
    omni_debug_log("VIN", "Task deletion failed!");
    assert(0);
    for (;;) { }
}
...
void omni_libvin_main(void) {
    if (!initialized) {
        omni_debug_log("VIN", "Initializing VIN reader");
        ...
        omni_debug_log("VIN", "Starting VIN reader task");
        ...
        initialized = true;
        omni_debug_log("VIN", "VIN reader initialization complete");
    }
}
...
bool omni_libvin_get_vin(char buf[17]) {
    ...
    if (xSemaphoreTake(shared_data.semaphore, 0) == pdTRUE) {
        ...
        if (shared_data.vin[16]) {
            omni_debug_log("VIN", "Retrieved VIN: %.17s", shared_data.vin);
            // ESP_LOGD(tag, "vin read complete");
            memcpy(buf, shared_data.vin, sizeof(shared_data.vin));
            xSemaphoreGive(shared_data.semaphore);
            return true;
        }
        omni_debug_log("VIN", "VIN not yet available");
        // ESP_LOGD(tag, "vin read incomplete");
        xSemaphoreGive(shared_data.semaphore);
        return false;
    }
    omni_debug_log("VIN", "Could not acquire mutex for VIN read");
