...
static void get_next_event(struct isotp_event* evt) {
    assert(evt);
    omni_debug_log("ISOTP", "Waiting for next event");
    BaseType_t ret = xQueueReceive(isotp_event_queue_handle, evt, portMAX_DELAY);
    assert(ret == pdTRUE);
    omni_debug_log("ISOTP", "Received event type: %d", evt->type);
}

static void unmatched_frame(const uint8_t* frame) {
    assert(frame);
    omni_debug_log("ISOTP", "Processing unmatched frame");
    // NOTE: if this queue is full the frame will be dropped!
    // xQueueSend(isotp_unmatched_frame_queue_handle, frame, 0);
    if (xQueueSend(isotp_unmatched_frame_queue_handle, frame, 0) != pdTRUE) {
        omni_debug_log("ISOTP", "Unmatched frame dropped - queue full");
    }
}
...
static void write_frame(uint32_t id, uint8_t dlc, const uint8_t* data) {
    ...
    if (msg.extd) {
        omni_debug_log("ISOTP", "TX EXT: ID=%08lx DLC=%d DATA=%02X %02X %02X %02X %02X %02X %02X %02X",
        ...
        omni_debug_log("ISOTP", "TX STD: ID=%03lx DLC=%d DATA=%02X %02X %02X %02X %02X %02X %02X %02X",
            (unsigned long)msg.identifier, msg.data_length_code,
            msg.data[0], msg.data[1], msg.data[2], msg.data[3],
            msg.data[4], msg.data[5], msg.data[6], msg.data[7]);
