...
static void can_dispatcher(void* ptr) {
    (void)ptr;
    omni_debug_log("CAN", "Dispatcher task started");
    for (;;) {
        struct twai_message_timestamp msg;
        if (xQueueReceive(can_msg_queue_handle, &msg, portMAX_DELAY) == pdTRUE) {
            omni_debug_log("CAN", "Dispatching message ID: 0x%lx", (unsigned long)msg.msg.identifier);
            if (handlers[0]) {
                handlers[0](&msg);
            }
            if (handlers[1]) {
...
__attribute__((optimize("Ofast"))) static void can_reader(void* ptr) {
    (void)ptr;
    omni_debug_log("CAN", "Reader task started");
    for (;;) {
        struct twai_message_timestamp msg;
        ...
        case ESP_OK: {
            if (msg.msg.extd) {
                omni_debug_log("CAN", "RX: ID=0x%08lX DLC=%d DATA=%02X %02X %02X %02X %02X %02X %02X %02X",
                    (unsigned long)msg.msg.identifier, msg.msg.data_length_code,
                ...
                omni_debug_log("CAN", "RX: ID=0x%03lX DLC=%d DATA=%02X %02X %02X %02X %02X %02X %02X %02X",
                    (unsigned long)msg.msg.identifier, msg.msg.data_length_code,
                    msg.msg.data[0], msg.msg.data[1], msg.msg.data[2], msg.msg.data[3],
                    msg.msg.data[4], msg.msg.data[5], msg.msg.data[6], msg.msg.data[7]);
...
static void reinstall(void) {
    omni_debug_log("CAN", "Reinstalling CAN driver");
    vTaskDelete(can_reader_handle);
    twai_stop();
    twai_driver_uninstall();
    if (twai_driver_install(&general_config, &timing_config, &filter_config) == ESP_OK) {
        // ESP_LOGI(tag, "driver installed");
        omni_debug_log("CAN", "Driver reinstalled successfully");
    } else {
        // ESP_LOGE(tag, "driver installation failed");
        omni_debug_log("CAN", "Driver reinstallation failed");
    }
    if (twai_start() == ESP_OK) {
        // ESP_LOGI(tag, "driver started");
        omni_debug_log("CAN", "Driver restarted successfully");
    } else {
        // ESP_LOGE(tag, "driver start failed");
        omni_debug_log("CAN", "Driver restart failed");
    }
...
void omni_libcan_main(void) {
    if (!initialized) {
        omni_debug_log("CAN", "Initializing CAN driver");
        if (twai_driver_install(&general_config, &timing_config, &filter_config) == ESP_OK) {
            // ESP_LOGI(tag, "driver installed");
            omni_debug_log("CAN", "Driver installed successfully");

        } else {
            // ESP_LOGE(tag, "driver installation failed");
            omni_debug_log("CAN", "Driver installation failed");
        }
        if (twai_start() == ESP_OK) {
            // ESP_LOGI(tag, "driver started");
            omni_debug_log("CAN", "Driver started successfully");
        } else {
            // ESP_LOGE(tag, "driver start failed");
            omni_debug_log("CAN", "Driver start failed");
        }

        omni_debug_log("CAN", "Creating message queue and tasks");
        ...
        initialized = true;
        omni_debug_log("CAN", "Initialization complete");
    }
}

void omni_libcan_add_incoming_handler(omni_libcan_incoming_handler* handler) {
    omni_debug_log("CAN", "Adding incoming handler: %p", (void*)handler);
    if (!handlers[0]) {
        handlers[0] = handler;
        omni_debug_log("CAN", "Handler added to slot 0");
    } else if (!handlers[1]) {
        handlers[1] = handler;
        omni_debug_log("CAN", "Handler added to slot 1");
    }else {
        omni_debug_log("CAN", "No free handler slots");
    }
}

void omni_libcan_add_filter(uint32_t id, bool extd) {
    omni_debug_log("CAN", "Adding filter - ID: 0x%lx, Extended: %d", (unsigned long)id, extd);
    ...
    omni_debug_log("CAN", "Updated filter=0x%lx, mask=0x%lx", (unsigned long)filter, (unsigned long)mask);
}

void omni_libcan_clear_filter(void) {
    omni_debug_log("CAN", "Clearing all filters");
    filter = 0xFFFFFFFF;
    filters_or = 0;
    mask = 0xFFFFFFFF;
}
